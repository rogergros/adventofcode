...
class History
    ...
    def resolve(verbose)
        ...
        last_difference = sequences.last.values.last
        while step > 0
            print "Last difference in level #{step}: #{last_difference}\n" if verbose
            step -= 1
            last_difference = sequences[step].values.last + last_difference
        end
        last_difference
    end
end
